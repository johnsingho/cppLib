...
//delete all, 如果有析构器的话，给每个元素调用析构器
...
//是否允许32bit的virtualization
//virtualization仅用于交互用户以非管理员权限执行的32位程序
...
//如果bWaitForExit==true, 那么poExitCode返回ExitCode
...
//不要时，使用 DeleteObject释放
JOHN_DLL HPALETTE Bitmap_CreateDIBPalette(void* pDIB);
//返回GlobalAlloc分配的内存，需要自己GlobalFree
...
// millisecond(1601.01.01) 与具体类型互转
// timLocal	1592216882
// ms	    13236719282000
...
//封装以上两个方法
//从pRawData复制数据到pData，不足blkLen的话，填充。再实施加密。
// <0 失败
// >0 写到pData的有效字节数
// blk和pRawData允许是同一个地址。
//
//// Sample:
/*
...
{
	ESSH_SHA256,    //32bytes结果，自动选择软硬件方式
	ESSH_SHA256_HW,
	ESSH_SHA256_SW,
	ESSH_SHA384,   //48bytes结果
	ESSH_SHA512,   //64bytes结果
	ESSH_SHA3_224, //28bytes结果
	ESSH_SHA3_256,
	ESSH_SHA3_384,
	ESSH_SHA3_512  //64bytes结果
}EHashKind;
